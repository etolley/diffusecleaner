...
class Antenna:
    ...
    def __init__(self, name = 'VLBA'):
        ...
        if name == 'VLBA' or name == 'vlba':
            filename = "vlba.txt"
        else:
            raise RuntimeError("Telescope configuration with name {0} does not exist.".format(name))

        X, Y, Z, _, _ = np.genfromtxt(file_dir / "layouts" / filename, unpack=True)
